...
```{r Read_data}
RNA_seq = read.csv("..//Low_hig_gene/Less_than_one_std_deviation.txt",row.names = 1,sep = "\t",check.names = FALSE)
...
readr::write_rds(bwnet,
                 file = file.path("Low_wgcna_results.RDS")
)
...
```{r gene_search}
#https://perslab.github.io/bentsen-rausch-2020/15_tany_wgcna_pseudo.html
modules<-MEs
...
color <- c("turquoise","blue","midnightblue","greenyellow")
lapply(color, function(col) {
  maxsize <- 50
  hubs <- data.frame(genes=hubgenes[[col]]$gene[1:maxsize], kme = hubgenes[[col]]$kme[1:maxsize], mod = rep(col,50))
}) -> hub_plot 
...
sigmods <- c("magenta","red")
sigmods
lapply(sigmods, function(col) {
    maxsize <- 40
    hubs <- data.frame(genes=hubgenes[[col]]$gene[1:maxsize], kme = hubgenes[[col]]$kme[1:maxsize], mod = rep(col,40))
}) %>% bind_rows() -> hub_plot1 
...
adj[hub_plot1$genes, hub_plot1$genes] %>% 
  ...
  dplyr::mutate(color = ifelse(name %in% c("SIRPA","ZBTB8A","TET1","PHPT1"), yes="red", no="black")) -> hub_plot1
...
geneplot <- ggraph(hub_plot1,  layout = 'fr') + 
  ...
  geom_node_point(aes(fill = color, size=kme), shape=21, alpha=0.5) + 
...
geneplot <- ggraph(hub_plot1,  layout = 'kk') + 
  ...
  scale_size(range = c(1,5), name = "kME") +
  scale_fill_manual(name="Module", values = c("turquoise","blue","midnightblue","greenyellow")) +
   # scale_fill_manual(name="Module", values = c("magenta","red")) +
...
pca.blue <- ggplot(pca2, aes(x = PC3, y = PC4,color = RISK_CYTO)) +
...
pca.turquoise <- ggplot(pca2, aes(x = PC3, y = PC4,color = RISK_CYTO)) +
  geom_point(aes(size = MEturquoise)) +
  ggtitle("MEturquoise") +
 # scale_color_viridis() +
  theme_classic(base_size = 30)+
 guides(colour = guide_legend(override.aes = list(size=10)))
...
pca.midnightblue <- ggplot(pca2, aes(x = PC3, y = PC4,color = RISK_CYTO)) +
  geom_point(aes(size = MEmidnightblue)) +
  ggtitle("MEmidnightblue") +
 # scale_color_viridis() +
  theme_classic(base_size = 30)+
 guides(colour = guide_legend(override.aes = list(size=10)))
  #theme(plot.title = element_text(hjust = 0.5), legend.title=element_blank()) 
...
pca.greenyellow <- ggplot(pca2, aes(x = PC3, y = PC4,color = RISK_CYTO)) +
  geom_point(aes(size = MEgreenyellow)) +
  ggtitle("MEgreenyellow") +
 # scale_color_viridis() +
  theme_classic(base_size = 30)+
 guides(colour = guide_legend(override.aes = list(size=10)))
  #theme(plot.title = element_text(hjust = 0.5), legend.title=element_blank()) 
...
pca.pink <- ggplot(pca2, aes(x = PC3, y = PC4,color = RISK_CYTO)) +
...
pca.tan <- ggplot(pca2, aes(x = PC3, y = PC4,color = RISK_CYTO)) +
...
pca.red <- ggplot(pca2, aes(x = PC3, y = PC4,color = RISK_CYTO)) +
...
pca.magenta <- ggplot(pca2, aes(x = PC3, y = PC4,color = RISK_CYTO)) +
